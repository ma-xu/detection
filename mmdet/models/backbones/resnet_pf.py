...
class ParamFree(nn.Module):
    def __init__(self, channel, groups=64,mode='dotproduct'):
        ...
        self.mode = mode
        self.groups = groups
        self.scale = Parameter(torch.zeros(1,self.groups,1))
        self.bias = Parameter(torch.ones(1, self.groups, 1))
    ...
    def forward(self, x):
        ...
        gap2 = self.gap2(x)
        gap4 = self.gap4(x)
        gapALL =(gap+gap4+F.interpolate(gap2,scale_factor=2))/3
        gapALL = F.interpolate(gapALL, size=(h,w))
        # print(gapALL.shape)
        x = x.view(b * self.groups, -1, h*w)
        gapALL = gapALL.view(b * self.groups, -1, h*w)
        similarity = (x*gapALL).sum(dim=1,keepdim=True)

        similarity = similarity.view(b, self.groups, h * w)
        similarity = similarity-similarity.mean(dim=2,keepdim=True)
        similarity = similarity/(similarity.std(dim=2, keepdim=True) + 1e-7)

        similarity = self.sig(similarity*self.scale+self.bias)
...
class ResNetPF(nn.Module):
    ...
    def train(self, mode=True):
        super(ResNetPF, self).train(mode)
